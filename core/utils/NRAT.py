...
def symmkl(logit1, logit2):
    ...
    loss_2 = criterion_kl(F.log_softmax(logit2, dim=1), 
...
def NRAT_loss(model, x_natural, y, optimizer, step_size=0.003, epsilon=0.031, perturb_steps=10, beta=4.0, 
    ...
    loss_natural = RL(logits_natural, y)  # NCERCE

    loss_robust = (1.0 / batch_size) * symmkl(logits_adv, logits_natural)      # symkl
